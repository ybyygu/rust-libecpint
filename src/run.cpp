...
void read_basis_file(std::string filename, DVec &exps, DVec &coeffs,
                     DVec &coords, IVec &ams, IVec &lens, Coord &atom) {
      ...
      while (sc_pos != std::string::npos) {
        ...
        if (len == -1) {
          ...
          if (c_pos != std::string::npos) {
            ...
            coeffs.push_back(
                std::stod(token.substr(c_pos + 1, token.length())));
            len++;
          }
        }
        sc_pos = line.find(';');
...
ECPData test_ecpint(std::string share_dir) {
  ...
  // hydrogen cc-pVDZ
  read_basis_file("hydrogen.bas", g_exps, g_coeffs, g_coords, g_ams, g_lens,
                  H_pos);
  // iodine cc-pVDZ-PP
  read_basis_file("iodine.bas", g_exps, g_coeffs, g_coords, g_ams, g_lens,
                  I_pos);
  // check sizes, should read 'Basis read: 44, 12, 36'
  std::cout << "Basis read: " << g_exps.size() << ", " << g_ams.size() << ", "
            << g_coords.size() << std::endl;

  // Now to perform a calculation using the high-level API we do the
  // following:
  ECPIntegrator factory; // object used to build ECP integral matrices

  // Set the orbital basis sets
  factory.set_gaussian_basis(g_ams.size(), g_coords.data(), g_exps.data(),
                             g_coeffs.data(), g_ams.data(), g_lens.data());
  // put an ECP on the iodine (charge = 53, name="ecp28mdf")
  std::vector<std::string> names = {"ecp28mdf"};
  int charges[1] = {53};
  factory.set_ecp_basis_from_library(1, I_pos.data(), charges, names,
                                     share_dir);
